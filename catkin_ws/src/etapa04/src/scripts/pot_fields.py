...
def callback_pot_fields_goal(msg):
	if not msg.dir:
		...
		tolerance = 0.1
		epsilon = 0.5
		while dist_to_goal > tolerance and not rospy.is_shutdown() and rospy.get_param('/pot_fields_go',True):
		...
		if rospy.get_param('/pot_fields_go',True):
			rospy.set_param('/reached_flag', True)
...
def main():
    ...
    rospy.Subscriber('/path_or_field', path, callback_pot_fields_goal)
