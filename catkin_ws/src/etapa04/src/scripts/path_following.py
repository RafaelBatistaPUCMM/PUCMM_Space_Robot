...
def follow_path(path):
    ...
    while not rospy.is_shutdown() and global_error > 0.1 and rospy.get_param('/path_planning_go',True):
...
def callback_global_goal(msg):
	if msg.dir:
		...
		if rospy.get_param('/path_planning_go',True):
...
def main():
    ...
    rospy.Subscriber('path_or_field', path, callback_global_goal)
