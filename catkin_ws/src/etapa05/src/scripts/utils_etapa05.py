...
class TF:
	...
	def send_transform(self, transl, rot, frame, ref=None):
		...
		for i in range(len(self._trans_list)):
			trans = self._trans_list[i]
			if trans.child_frame_id == tfs.child_frame_id:
				self._trans_list.pop(i)
				break
...
class RGBD:
	...
	def __init__(self):
		...
		self._head = moveit_commander.MoveGroupCommander('head',wait_for_servers=20)
		arm =  moveit_commander.MoveGroupCommander('arm',wait_for_servers=20)
	...
	def _control_task(self):
		...
		while True:
			if self.ControlMode == 1: 	#Angle Control	
				...
				if abs(self.error[0]) > 0.50:
					KP[0] = 0.2/abs(self.error[0])
				else:
					KP[0] = 0.8
				
				if abs(self.error[1]) > 0.50:
					KP[1] = 0.2/abs(self.error[1])
				else:
					KP[1] = 0.8
...
class PickPlace:
	def __init__(self, cntrl, offset = 0.3):
		self.offset = 0.3
		self.arm =  moveit_commander.MoveGroupCommander('arm', wait_for_servers=20)
		self.wb = moveit_commander.MoveGroupCommander('whole_body_light', wait_for_servers=20)
		self.cntrl = cntrl

		#TF
		self.TF_obj = TF('odom', 'hand_palm_link')

		#Gripper
		self._grip_cmd_pub = rospy.Publisher('/hsrb/gripper_controller/command', trajectory_msgs.msg.JointTrajectory, queue_size=100)
		self._grip_cmd_force = rospy.Publisher('/hsrb/gripper_controller/grasp/goal', tmc_control_msgs.msg.GripperApplyEffortActionGoal, queue_size=100)
	...
	def change_grip_vel(self, vel):
		self._velocity = vel
    
	def _grip_state(self, state):
		if state:
			self._position = -0.82
			self._effort = -0.3
		else:
	...
	def _move_arm(self, coord):
		self.TF_obj.set_obj('hand_palm_link')
		trans, rot = self.TF_obj.get_transform_array()
		rot = quaternion_from_euler(rot[0], rot[1], rot[2])
		self.TF_obj.send_transform(coord, rot, 'goal')
		self.TF_obj.set_obj('goal')
		trans, rot = self.TF_obj.get_transform_array()
		rot = quaternion_from_euler(rot[0], rot[1], rot[2])
		...
		self.wb.set_start_state_to_current_state()
		self.wb.set_pose_target(pose_goal)
		result = self.wb.go(wait=True)
		print("Stop", result)
		self.wb.stop()
		self.wb.clear_pose_targets()
		return result
		
	def grip(self, coord):
		self._grip_state(False)
		if not self.approach(coord, False):
			return False
	...
	def release(self, coord):
		if not self.approach(coord.copy(), False):
			return False
		self._grip_state(False)
		if not self.approach(coord.copy(), True):
			return False
		self.go_default()
		return True
	
	def approach(self, coord, off_state):
		target_offset = 0.8
		#self.TF_obj.set_obj('hand_palm_link')
		#trans, rot = self.TF_obj.get_transform_array()
		target = get_gaze_target(self.cntrl.pos, [coord[0], coord[1]], target_offset)
		
		"""
		#Correcting angle to target
		self.cntrl.pos.final = target
		angle = self.cntrl.pos.find_angle()
		self.cntrl.start_control(1,angle)
		while self.cntrl.is_controlling():
			None
		"""
		
		#Getting to target
		self.cntrl.start_control(2,target)
		while self.cntrl.is_controlling():
			None
		
		#Correcting angle to point
		self.cntrl.pos.final = target
		angle = self.cntrl.pos.find_angle()
		self.cntrl.start_control(1,angle)
		while self.cntrl.is_controlling():
		...
		if off_state:
			self.TF_obj.set_obj('hand_palm_link')
			trans, rot = self.TF_obj.get_transform_array()
			target = get_dist_target(trans, newCoord, self.offset)
		else:
			target = newCoord
...
def get_dist_target(ref, point, offset):
	point = np.array(point)
	coords = np.array(ref)
	displaz = point - coords
	distance = np.linalg.norm(displaz)
...
def ang_sweep(cntrl):
	angle = cntrl.pos.get_current_angle()
	for i in range(0,4):
		angle += np.pi/2
		if angle > np.pi:
			angle -= 2*np.pi
